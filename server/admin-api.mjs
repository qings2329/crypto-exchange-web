...
//         （默认监听 :8080；Vite 代理 /api -> :8080，故直接作为开发后端即可）
//
...
// ---------- 网关鉴权（真实网关模拟）----------
// 1) 公开端点（登录/刷新/登出）先于网关注册，避免被拦截。
app.use(authRouter);
// 2) 网关：校验除公开端点外的所有 /api/v1 请求的 Bearer 令牌（401）。
app.use(gateway);
// 3) 当前用户档案（需鉴权，由网关保障）；随后各业务前缀施加服务端 RBAC。
app.get("/api/v1/user/me", (req, res) => res.json({ code: 0, message: "ok", data: { user_id: req.user.sub, role: req.user.role, username: req.user.username } }));
// 4) 服务端 RBAC：admin 业务前缀仅 admin 可访问；otc 为运营及以上（operator/admin）可访问。
app.use(
...
if (isMain) {
  app.listen(PORT, () => {
    console.log(`[admin-api] mock server listening on :${PORT} (base /api/v1/{risk,notification,futures,options,margin,admin})`);
  });
}
